...
*/
    ...
    uint256[] public activeAuctions; // Array to store active auction identifiers (e.g., tokenIds)
...
*/
    ...
    modifier isAuctionWinner(
        ...
        if (sender != nftContractAuctions[nftAddress][tokenId].currentWinner) {
    ...
    modifier isAuctionNftSeller(
        ...
        if (sender != nftContractAuctions[nftAddress][tokenId].nftSeller) {
    ...
    modifier isAuctionBidded(address nftAddress, uint256 tokenId) {
        if (
            nftContractAuctions[nftAddress][tokenId].minPrice !=
            nftContractAuctions[nftAddress][tokenId].temporaryHighestBid
        ) {
    ...
    modifier isAuctionNotBidded(address nftAddress, uint256 tokenId) {
        if (
            nftContractAuctions[nftAddress][tokenId].minPrice ==
            nftContractAuctions[nftAddress][tokenId].temporaryHighestBid
        ) {
...
*/
    ...
    function InitializeAuction(
        ...
        console.log("4");
        activeAuctions.push(_tokenId); // Add the new auction to active auctions
...
*/
        ...
        isAuctionWinner(_nftContractAddress, _tokenId, msg.sender)
    {
        ...
        removeActiveAuction(_tokenId); // Remove the auction from active auctions
    ...
    // TODO: This function can be called multiple times by the seller. We need to make sure that the seller can only call this function once.
    ...
    function removeActiveAuction(uint256 _tokenId) internal {
...
*/
    ...
    function getTemporaryHighestBid(
        address _nftContractAddress,
        uint256 _tokenId
    ...
    function getIntervalOfNftAuction(
        address _nftContractAddress,
        uint256 _tokenId
    ...
    function getBeginningPriceOfTheNft(
        address _nftContractAddress,
        uint256 _tokenId
    ) public view returns (uint256) {
    ...
    function getStartingTimeOfAuction(
        address _nftContractAddress,
        uint256 _tokenId
    ) public view returns (uint256) {
        return
            nftContractAuctions[_nftContractAddress][_tokenId].s_lastTimeStamp;
    }

    //This function will return whether the Auction is ongoing or not

    function getStateOfAuction(
        address _nftContractAddress,
        uint256 _tokenId
    ...
    function getSpecificAddress(
        ...
        return
            nftContractAuctions[_nftContractAddress][_tokenId].s_bidders[index];
    }

    //This function will return the bid made by  an specific address

    function getBidOfAnAddress(
        address _nftContractAddress,
        uint256 _tokenId,
        address sender
    ) public view returns (uint256) {
    ...
    function getAmountFundedByAnAddress(
        address _nftContractAddress,
        uint256 _tokenId,
        address sender
    ) public view returns (uint256) {
        return
            nftContractAuctions[_nftContractAddress][_tokenId]
                .s_addressToAmountFunded[sender];
    }
